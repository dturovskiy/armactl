...
This section is a design contract for runtime behavior. Phase 4b adds a `player_sessions` schema/vocabulary foundation in `players.db`, Phase 4c adds explicit service-layer open/close writers, and Phase 4d-a adds explicit stored-log sessionization over already persisted `player_log_events`. The current code still has no live scanner, no automatic poller, no retention job, no heuristic disconnect pairing, and no hidden writes from player GET routes.
...
| --- | --- | --- | --- | --- |
| Player registry DB | `~/armactl-data/<instance>/players.db` | `players`: reliable ID, current name, first/last seen, seen count, last source. `player_names`: reliable ID, name, first/last seen, seen count. `player_log_events`: deduped parsed event kind, observed/log timestamp, source/ref/confidence, sanitized identity/name/session/faction/combat fields. `player_sessions`: Phase 4b schema/vocabulary, Phase 4c explicit service writer foundation, and Phase 4d-a stored-log open/update observations with checkpoint metadata. Schema meta. | IPs/player addresses, raw log lines, raw absolute paths, secrets, RCON password, admin password, complete live online/offline truth, heuristic disconnect pairings, ban/kick history, Discord K/D projections, raw RCON rows. | No player retention/cleanup currently. File is forced to mode `0600`. |
