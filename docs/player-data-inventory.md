...
| Player session writer helpers (`player_registry.observe_player_session`, `player_registry.close_player_session`) | Explicit service-layer caller evidence with normalized reliable IDs; Phase 4d-a/4d-b invoke observe/close only from already stored log events | `players.db` tables `player_sessions`, plus `players`/`player_names` identity observations | Only when an explicit service/job caller invokes the helper; no automatic freshness | High/moderation data | Opens/updates one open session per reliable ID and closes open sessions with sanitized source/ref/confidence/end-reason/faction/side/correlation fields. Rejects unreliable IDs and stores no IPs, raw log lines, raw absolute paths, public IDs, or secrets. Not a live scanner, poller, retention job, UI/API page, current-roster refresh side effect, or current-session K/D/role/joined source. |
...
This section is a design contract for runtime behavior. Phase 4b adds a `player_sessions` schema/vocabulary foundation in `players.db`, Phase 4c adds explicit service-layer open/close writers, Phase 4d-a adds explicit stored-log open/update sessionization over already persisted `player_log_events`, and Phase 4d-b adds stored-log close/lifecycle handling for unambiguous correlation and server-boundary evidence. The current code still has no live scanner, no automatic session/log poller, no retention job, no stale-close window, no session UI/API, and no hidden registry/session writes from player GET routes.
...
| --- | --- | --- | --- | --- |
| Player registry DB | `~/armactl-data/<instance>/players.db` | `players`: reliable ID, current name, first/last seen, seen count, last source. `player_names`: reliable ID, name, first/last seen, seen count. `player_log_events`: deduped parsed event kind, observed/log timestamp, source/ref/confidence, sanitized identity/name/session/faction/combat/disconnect/lifecycle fields. `player_sessions`: Phase 4b schema/vocabulary, Phase 4c explicit service writer foundation, Phase 4d-a stored-log open/update observations, and Phase 4d-b safe stored-log close/lifecycle observations. Schema meta. | IPs/player addresses, raw log lines, raw absolute paths, secrets, RCON password, admin password, complete live online/offline truth, stale-close scanner state, ambiguous/name-only disconnect pairing, ban/kick history, Discord K/D projections, raw RCON rows. | No player retention/cleanup currently. File is forced to mode `0600`. |
...
- Read-only stored event history is implemented for `player_log_events`, Phase 4b adds the `player_sessions` schema foundation, Phase 4c adds explicit service writer helpers, Phase 4d-a adds explicit stored-log open/update sessionization, and Phase 4d-b adds safe stored-log close/lifecycle handling; richer session/history semantics still need a live scanner job, stale-close handling, retention policy, and conflict handling before complete live online/offline/session truth is produced.
- Phase 4a now distinguishes reliable connect/session signals from heuristic disconnect pairing, roster deltas, A2S count hints, and mod-dependent combat events; Phase 4d-a enforces the stored-log open/update subset and Phase 4d-b accepts only unambiguous stored close/lifecycle evidence, while stale closes, retention, live scanner conflict windows, session UI/API, and richer current-session semantics remain future work.
...
- Implement the remaining Phase 4a freshness/conflict rules for A2S count, RCON roster, live scanner checkpoints, stale closes, and disconnect/lifecycle boundaries. A2S cannot identify players; RCON can identify some players but can be unavailable.
