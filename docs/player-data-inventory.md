...
| Stored-log sessionization job (`players:sessionize-log-events`) | Already stored sanitized `player_log_events` in the instance `players.db`; no live log reader | Existing `player_sessions` rows through Phase 4c helpers, with per-session checkpoint metadata | Explicit background job only; active queued/running jobs dedupe | High/moderation data | Phase 4d-a opens/updates sessions from auth/update reliable IDs with high confidence, and uses faction/combat stable UUIDs only as inferred presence/faction evidence. Phase 4d-b closes open sessions only from unambiguous stored RPL identity, connection ID, or slot correlation, or from accepted server-boundary lifecycle markers. Job output and audit details are counts-only. It does not read live logs, poll A2S/RCON, open identified sessions from player counts, close stale sessions, close from ambiguous/name-only disconnects, run retention cleanup, expose session UI/API pages, store IP/raw path/raw line/secrets, or claim joined time, role, K/D, ban/kick, or Discord enrichment. |
| Explicit live session scan (`scan_live_player_sessions_once`, `players:scan-live-sessions`) | Existing safe current-roster source through `player_sources.load_current_player_roster`; RCON roster rows only when they carry reliable IDs; A2S remains count-only | Existing `player_sessions` rows through `player_registry.observe_player_session`, plus identity observations performed by that helper | Explicit service/helper or background job run only; active queued/running jobs dedupe | High/moderation data | Phase 4e-a opens/updates medium-confidence live presence observations for reliable roster IDs. Unreliable/name-only rows and A2S count-only state are counted and ignored. Source failure or roster unavailability does not close sessions; stale-close and retention remain separate explicit maintenance. Job output/audit details are counts-only. It does not add an automatic daemon/poller/scheduler, GET mutation, web button/route, session UI/API, IP/raw path/raw line/secret storage, current-session joined/playtime/K-D/role/faction truth, ban/kick, or Discord enrichment. |
...
- Read-only stored event history is implemented for `player_log_events`, Phase 4b adds the `player_sessions` schema foundation, Phase 4c adds explicit service writer helpers, Phase 4d-a adds explicit stored-log open/update sessionization, Phase 4d-b adds safe stored-log close/lifecycle handling, Phase 4d-c adds explicit stale-close/retention maintenance, and Phase 4e-a adds an explicit one-shot live roster scanner; richer session/history semantics still need automatic scheduling, conflict windows, and UI/API truth labels before complete live online/offline/session truth is produced.
- Phase 4a now distinguishes reliable connect/session signals from heuristic disconnect pairing, roster deltas, A2S count hints, and mod-dependent combat events; Phase 4e-a accepts only reliable roster IDs as medium-confidence live presence evidence, while automatic pollers, live scanner conflict windows, session UI/API, and richer current-session semantics remain future work.
- Manual explicit log-file CLI import exists for bounded text files, and manual web collection exists only for allowlisted current-instance config console logs. Future work still needs to decide automatic live scanner/dashboard poll triggers; the current code has no automatic session recorder or poller.
- Implement the remaining Phase 4a freshness/conflict rules for A2S count, repeated RCON absence, live scanner checkpoints, stale closes, and disconnect/lifecycle boundaries. A2S cannot identify players; RCON can identify some players but can be unavailable.
...
- Slice 3: automatic live scanner/sessionization, conflict windows, session UI/API, and retention scheduling on top of the parser/import/storage/session-schema/session-writer/live-scan foundation, with no IP storage by default.
