...
- `armactl players current-cache run` persists only a sanitized current-roster snapshot in `web.db`. It is an availability/cache optimization for web polling, not online/offline, joined-time, role, faction, or current-session K/D evidence.
