...
- [ ] Player-session F3-b supervised pipeline implementation: replace enqueue-only automatic scheduling with one synchronous ordered oneshot/timer, shared cross-process pipeline lock, completion-based status, bounded generation consumption, counts-only output, explicit install/enable/disable/status, and no dependency on `armactl-web.service` or `web_jobs` execution.
