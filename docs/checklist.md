...
- [ ] Player-log Slice F2-b VM deployment and acceptance: Serhiivka first, explicitly install and enable the F2-a timer, observe repeated non-overlapping cycles and stored freshness without pressing the manual button, confirm no `armareforger.service` restart/mutation and no player-session scheduler enablement, then proceed to Chervonopilya only after explicit approval. Stats still require a proven open play session.
