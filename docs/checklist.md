...
- [x] Current players table UX cleanup on /players, with compact Player/Status, placeholder Kills/Deaths/TK/Faction/Role cells rendered as `—`, Available actions, sanitized ID/source/freshness hidden behind closed Details actions, no GET/player DB/session writes, and K/D/deaths/teamkills/faction/role/joined time gated behind explicit player stats truth/enrichment.
...
- [x] Player stats truth audit for future current-player `Kills`/`Deaths`/`TK`/`Faction`/`Role`/first-observed columns, with source matrix, verdicts, blocked/allowed slices, and explicit exclusions for fake zeroes, K/D, Discord enrichment before web truth, GET writes, public player IDs, and raw log/path/IP/secret exposure.
...
- [x] Player stats truth Slice D play-session/reconnect model foundation, with durable play-session window metadata, server-run lifecycle boundary markers, 10-minute compatible reconnect merge, identity-conflict blocking, last gameplay evidence metadata, current-player stat placeholders preserved, and no GET mutation, daemon/timer, Discord/public enrichment, fake stats, K/D column, or role truth.
...
- [x] Add the initial web-only current-player enrichment slice: read-only aggregation over existing player_log_events and open player_sessions for authenticated /players placeholders, with nullable labels/placeholders, Kills/Deaths/TK/Faction/Session first observed fields, no DB migration/materialized counters/jobs, no K/D, no Discord/public enrichment, Role still placeholder, no stats/session GET writes, no public player IDs, and no raw log/path/IP/secret exposure. This is not final accepted session-stat truth until the play-session contract is implemented.
- [ ] Implement Player stats truth Slice E session-scoped current stats aggregation: use Slice C freshness and Slice D play-session windows, count only scoped reliable combat evidence, keep fake zeroes blocked unless proven, and preserve no K/D, no Discord/public enrichment, no role truth, and no GET mutation.
