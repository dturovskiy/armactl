...
- [x] Player-session F3-b supervised pipeline implementation: one synchronous ordered orchestrator, shared per-instance nonblocking mutation lock, atomic fixed ingest-generation high-water, bounded sessionization over that event-ID range in trusted occurrence-time order, versioned durable time cursor with fail-closed legacy-prefix promotion, exact reliable-roster gate, durable completion/interruption state, generated disabled-by-default oneshot/timer, explicit install/enable/disable/status, counts-only sanitized output, and no automatic dependency on `armactl-web.service` or `web_jobs` execution.
- [ ] Player-session F3-c staged production acceptance: Serhiivka first, then Chervonopilya only after pass evidence and explicit approval, with real player enter/faction/combat/disconnect/reconnect/grace/lifecycle/stat-split scenarios, no manual log/session buttons, no fake zeroes or duplicates, and no game restart merely for the test.
...
- [ ] After F3-b/F3-c supervision acceptance, decide any separate richer session UI/detail/API, search, and conflict-policy hardening beyond the read-only list and current truth labels, with no IP storage by default.
