...
- [x] Current players table UX cleanup on /players, with compact Player/Status, placeholder Kills/Deaths/TK/Faction/Role cells rendered as `—`, Available actions, sanitized ID/source/freshness hidden behind closed Details actions, no current JSON contract change, no GET/player DB/session writes, and K/D/deaths/teamkills/faction/role/joined time left future until reliable player stats truth/enrichment exists.
...
- [ ] Add the next web-only current-player truth slice: read-only aggregation over existing `player_log_events` and `player_sessions` for authenticated `/players` placeholders, with labels/placeholders, no DB migration/materialized counters/jobs, no K/D, no Discord/public enrichment, no stats/session GET writes, no public player IDs, and no raw log/path/IP/secret exposure.
