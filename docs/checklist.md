...
- [x] Player-session F3-b supervised pipeline implementation: one synchronous ordered orchestrator, shared per-instance nonblocking mutation lock, atomic ingest-generation high-water, bounded event-ID sessionization with fail-closed time validation, exact reliable-roster gate, durable completion/interruption state, generated disabled-by-default oneshot/timer, explicit install/enable/disable/status, counts-only sanitized output, and no automatic dependency on `armactl-web.service` or `web_jobs` execution.
