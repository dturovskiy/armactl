...
- [ ] Continue live session tracking with automatic poller/scheduler decisions, full session UI/detail/API beyond the read-only list, remaining retention scheduling, richer truth labels, and broader conflict-policy hardening from the Phase 4a design, with no IP storage by default.
