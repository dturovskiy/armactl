...
F2 automatic log freshness and F3 supervised player-session execution are accepted on both target VMs. The production path uses the explicit synchronous systemd oneshot/timer, not queued `web_jobs`, browser/GET triggers, or a hidden worker. The next player decision gate is separate richer session UI/detail/API and search/conflict-policy hardening; Discord/public enrichment remains later work and still requires an explicit truth/scope decision. Fake zeroes, hidden threads, raw sensitive evidence, and historical oversized-log backfill remain forbidden or separate decisions.
