...
Phase 4e/4f F3-b session supervision is explicit opt-in and synchronous. `armactl players sessions scheduler run --once` executes one ordered orchestrator directly, creates no automatic `web_jobs`, starts no web worker, and records terminal success only after durable completion. It proves a newer completed fresh ingest generation and exact event high-water, sessionizes bounded `event_id` pages with fail-closed trusted-time validation, performs one exact reliable live RCON scan, and runs maintenance only when due. One per-instance nonblocking mutation lock is shared with the existing manual POST job handlers, which retain `web_jobs` only as operator workflow metadata. The generated oneshot/timer is disabled and inactive after first install, preserves enablement on reinstall, uses a 150-second initial offset, 120-second completion-relative cadence, and 240-second failure guard, and has explicit install/enable/disable/status commands. Legacy scheduler rows remain read-only compatibility diagnostics. Automatic closes remain limited to unambiguous stored disconnect correlation, accepted server-boundary markers, repeated successful reliable RCON absence, or explicit stale-timeout maintenance. A2S/FPS remain count-only; names, IDs, paths, raw RCON/log content, IPs, secrets, source refs, public IDs, K/D, role, playtime, current faction truth, Discord enrichment, ban/kick, and banlist behavior remain forbidden. F3-c production installation, soak observation, acceptance, and rollback proof remain pending; no session timer or richer current-session UI/API is installed or enabled by this change.

The next automatic session slice is F3-c production acceptance, not a hidden poller or queue-only wrapper. It must install and explicitly enable only the reviewed generated units, verify ingest readiness and ordering, observe a supervised soak window, prove automatic session creation/closure/maintenance, and retain rollback. Full session detail/API and richer current-session semantics remain separate later work after F3-c acceptance. Exact join time, roles, playtime, K/D, public/player-facing stats, and Discord/public enrichment remain blocked.
