...
Phase 3e automatic current-roster cache updater foundation adds the shared safe `web.db` current-roster cache and the foreground CLI/service loop `armactl players current-cache run` with `--once`, default 10s interval, and redacted retry warnings. `/players` and `/players/current.json` now read fresh memory cache first, then fresh persistent safe cache, then bounded live fallback; successful live fallback updates only the safe current-roster cache, and live failures can serve a stale persistent snapshot with safe stale/error metadata. The cache stores sanitized snapshot metadata and player rows only: instance, collected/updated timestamps, source, status/error text, display names, reliable IDs, and row source. It does not write `players.db`, `players`, `player_names`, `player_sessions`, raw RCON output, raw paths, raw log lines, IPs, secrets, joined time, current role, exact faction truth, current-session K/D, ban/kick state, or Discord enrichment. This slice intentionally does not add JS polling; a next UI slice can connect `/players/current.json` polling at 10s with no-store fetch and graceful stale/error display.
...
Full last-seen automation beyond explicit background refresh, the snapshot-only current-roster cache updater, explicit service helper calls, and the Phase 4d-a/4d-b stored-log job, plus current-session semantics, should be added as a later session poller/service slice. That slice should define retention, stale-close rules, live scanner conflict windows, and disconnect boundaries before promoting join time, roles, or combat counters as current-player facts.

Future player history implementation should follow the Phase 4a session design before adding automatic session/log poller triggers, live sessionization, retention jobs, or richer session/history semantics, and before any public/Discord enrichment.
...
Do not add ban/kick mutations, aggregate kill/death stats, IP tracking, live journal readers, automatic session/log pollers, live sessionization, retention jobs, or Discord enrichment until later slices explicitly choose those sources and retention rules. Manual operator-triggered log collection is limited to the allowlisted background job above; current-roster registry refresh is limited to the explicit `players:refresh-current` job; automatic current-roster cache refresh is limited to the safe `players current-cache run` snapshot cache; stored-log sessionization and stored-log close/lifecycle handling are limited to `players:sessionize-log-events` and are not full live session tracking.
