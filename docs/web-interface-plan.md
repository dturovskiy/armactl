...
- Current players table UX cleanup on /players, with a compact Player/Status/Available actions default roster, sanitized identity/source/freshness fields hidden behind closed Details actions, no current JSON contract change, no GET/player DB/session writes, and no K/D/deaths/teamkills/faction/role/joined-time/Discord enrichment claims.
...
Phase 3e current-roster UX cleanup is UI-only. The default /players roster table shows Player, Status, placeholder `Kills`, `Deaths`, `TK`, `Faction`, and `Role` columns, plus Available actions; placeholder stat/faction/role cells render `—`. Closed Details rows contain sanitized reliable ID, source, freshness, and a `Joined time` placeholder that also renders `—`. This does not change /players/current.json, write players.db or sessions, or add current-session truth. K/D, deaths, teamkills, faction, role, and joined time remain future work until reliable player stats truth and enrichment are implemented.

The Player Stats Truth Audit is docs-only and lives in [player-data-truth-remediation-plan.md](player-data-truth-remediation-plan.md). It finds that stored combat events can support future `Kills`, `Deaths`, and `TK` only after a web-only read-only aggregation slice; `Faction` and `Joined time` can be shown only as last-known/first-observed evidence with clear labels; `Role` is blocked because no reliable source exists. The next allowed implementation slice is authenticated web-only current-player truth enrichment over existing `player_log_events` and `player_sessions`, using a separate read-only aggregation service, no DB migration, no materialized counters, no fake zeroes, no K/D column, no Discord/public enrichment, no stats/session GET writes, no public player IDs, and no raw log lines/paths/IPs/secrets.
...
- slice 3: web-only current-player truth enrichment from existing `player_log_events` / `player_sessions`, with read-only aggregation, labels, placeholders, no schema migration, no K/D, and no Discord/public enrichment;
- slice 4: implement full automatic session tracking, session detail/API, live scanner/sessionization scheduling, and retention policy using the Phase 4a design plus the parser/import/storage/session-schema/session-writer foundation, with no IP storage by default;
- slice 5: richer search/filter over reliable IDs, names, and session metadata;
- slice 6: audited banlist manager after source-of-truth, rollback, and identity rules are settled;
- slice 7: Discord stats enrichment after stable authenticated web player truth exists.
