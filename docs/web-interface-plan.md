...
- slice 6b: complete — query-only detail/search DTO foundation, alias-safe search, fail-closed time/source filters, bounded session/event keyset pagination, shared open/closed session stats, and focused regressions; Slice 6c authenticated UI and Slice 6d staged VM smoke remain separate and open;
...
Player-session Slice 6b is complete at the reusable query/DTO layer. The next player slice is Slice 6c: integrate the existing foundation through a thin authenticated server-rendered detail route/template and list cursor/filter UX without adding a JSON API or duplicating stats/query logic. Slice 6d remains a separate Serhiivka-first, approval-gated Chervonopilya VM smoke; no deployment or service restart is part of Slice 6b. Separately, close the public docs trim/move/sanitize slice and keep Discord/player enrichment and banlist/moderation behind their existing truth/recovery gates.
