...
The Player Stats Truth Audit lives in [player-data-truth-remediation-plan.md](player-data-truth-remediation-plan.md). The initial authenticated web-only enrichment slice uses a separate read-only aggregation service over existing player_log_events and player_sessions, with no DB migration, materialized counters, K/D column, Discord/public enrichment, stats/session GET writes, public player IDs, or raw log lines/paths/IPs/secrets. It is not final accepted session-stat truth. The required follow-up contract lives in [player-session-stats-contract.md](player-session-stats-contract.md): F2-a now supplies the local explicit supervised ingest foundation, while F2-b still must prove deployed automatic freshness; play-session boundaries, reconnect grace, parser fixture stability, no fake zeroes, and Discord/public gating remain authoritative. Kills count normal kills only after the session contract is satisfied, teamkills appear under TK, Faction is last-known stored evidence, Session first observed is not exact joined time, and Role is still blocked because no reliable source exists. Stats remain unavailable without a proven open play session, and F2-a does not enable the separate session scheduler.
...
Full last-seen automation beyond explicit background refresh, the snapshot-only current-roster cache updater, explicit service helper calls, the Phase 4d-a/4d-b stored-log job, the Phase 4e-a/4e-b one-shot live scan job, the authenticated web-only current-player enrichment, and the read-only stored-session list, plus full session detail/API and richer current-session semantics, should be added as a later session poller/service slice. That slice should define automatic scheduling, UI/API truth labels, and broader conflict policies before promoting exact join time, roles, playtime, K/D, public/player-facing stats, or Discord/public enrichment.
...
- slice 4: complete F2-b VM deployment/observation for the F2-a supervised ingest timer, then continue proving parser fixture stability, reconnect grace, reliable play-session boundaries, and session-scoped Kills/Deaths/TK/Faction evidence with no fake zeroes;
- slice 5: implement full automatic session tracking, session detail/API, live scanner/sessionization scheduling, and retention policy using the Phase 4a design plus the parser/import/storage/session-schema/session-writer foundation, with no IP storage by default;
...
For player-log freshness, F2-a is locally complete and the next step is F2-b VM deployment: Serhiivka first, explicitly install and enable `armactl-player-log-ingest.timer`, observe repeated non-overlapping 120-second completion-relative cycles and stored freshness without the manual button, verify no game-service restart/mutation and no player-session scheduler enablement, then consider Chervonopilya only after explicit approval. Separately, close the public docs trim/move/sanitize slice and decide which dashboard pieces remain clean public-core backports versus future private armactl-dashboard baseline. Keep Discord/player enrichment, banlist/moderation, and automatic session scheduling behind the existing truth/recovery gates.
