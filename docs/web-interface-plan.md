...
Phase 3e current-roster UX cleanup is UI-only. The default /players roster table shows only Player, Status, and Available actions; closed Details rows contain sanitized reliable ID, source, and freshness. This does not change /players/current.json, write players.db or sessions, or add current-session truth. K/D, deaths, teamkills, faction, role, and joined time remain future work until reliable player stats truth and enrichment are implemented.
