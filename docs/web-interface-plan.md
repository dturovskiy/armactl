...
- Read-only explicit player-session scheduler status through `armactl players sessions scheduler status`, showing the current enqueue-policy metadata from existing `web.db`; F3-a proved that `run --once` only creates/reuses queued `web_jobs`, starts no worker, waits for no terminal result, and can be permanently suppressed by an orphaned queued row. The selected disabled-by-default synchronous service/timer correction is documented in [player-session-supervised-pipeline-contract.md](player-session-supervised-pipeline-contract.md); F3-b implementation and F3-c production acceptance remain pending, with no scheduler service, timer, daemon, app-start hook, JS trigger, or GET trigger installed/enabled yet.
...
Phase 4e/4f scheduler runner foundation is explicit opt-in and enqueue-only. `armactl players sessions scheduler run --once` checks due state in `web.db`, creates or reuses queued rows for `players:scan-live-sessions`, `players:sessionize-log-events`, and `players:session-maintenance`, then records scheduler success without calling `dispatch_job`, starting a worker, waiting for a terminal row, or checking stage output. No generic worker drains the queue. The only session worker starts are daemon threads launched by explicit POST action wrappers for newly created jobs; restarting `armactl-web.service` does not resume queued/running work. Queued rows have no lease or stale recovery and can remain active indefinitely, so active-job dedupe is not an execution guarantee. F3-a maps this gap and selects one F3-b architecture in [player-session-supervised-pipeline-contract.md](player-session-supervised-pipeline-contract.md): one disabled-by-default systemd oneshot/timer calls a synchronous orchestrator independent of the web process, gates on a newly completed fresh ingest generation, runs stored-log sessionization before reliable live scan and due maintenance, shares one cross-process pipeline lock with manual handlers, and records success only after actual completion. Existing `web_jobs` remain manual workflow metadata. Automatic closes remain limited to unambiguous stored disconnect correlation, accepted server-boundary markers, repeated successful reliable RCON absence, or explicit stale-timeout maintenance. A2S/FPS remain count-only; names, IDs, paths, raw RCON/log content, IPs, secrets, source refs, public IDs, K/D, role, playtime, current faction truth, Discord enrichment, ban/kick, and banlist behavior remain forbidden. F3-b implementation and F3-c production acceptance are pending; no session scheduler service/timer or richer current-session UI/API is installed or enabled yet.

The next automatic session slice is F3-b, not a hidden poller or queue-only wrapper. It must implement only the supervised synchronous service/timer contract above; full session detail/API and richer current-session semantics remain separate later work after F3-c acceptance. Exact join time, roles, playtime, K/D, public/player-facing stats, and Discord/public enrichment remain blocked.
...
- slice 5: F3-b implement only the synchronous ordered player-session oneshot/timer from [player-session-supervised-pipeline-contract.md](player-session-supervised-pipeline-contract.md), then F3-c accept it on Serhiivka first and Chervonopilya only after explicit approval; richer session detail/API remains separate later work;
- slice 6: richer search/filter over reliable IDs, names, and session metadata;
...
Do not add ban/kick mutations, materialized aggregate counters, IP tracking, live journal readers, browser/GET/app-start/JS/hidden-thread session triggers, full session detail/API, or Discord enrichment until later slices explicitly choose those sources and truth labels. Manual operator-triggered log collection remains the allowlisted background job; F2 supervised ingest remains the explicit synchronous service/timer; current-roster registry refresh remains `players:refresh-current`; and automatic current-roster cache refresh remains the safe snapshot-only `players current-cache run`. Until F3-b is implemented, stored-log sessionization, live roster observation/repeated-absence close, and maintenance remain explicit manual jobs, and the current scheduler runner remains queue metadata rather than an execution guarantee. F3-b may automate only those existing session services through the documented synchronous ordered oneshot/timer; F3-c production enablement remains separate and approval-gated.
...
- F3-b synchronous supervised player-session service/timer implementation and F3-c staged production acceptance; richer session UI remains a separate later decision.
...
For player-log freshness, F2-a/F2-b/F2-c are complete: the explicitly enabled timer produced repeated non-overlapping fresh cycles on Serhiivka and Chervonopilya, the busy active log used bounded incremental catch-up, game/web processes remained unchanged, and the separate player-session scheduler stayed disabled. The next player slice is a complete automatic session-pipeline design and live-player acceptance: connect the existing conservative session jobs through an explicit supervised contract, then prove reliable open/close/reconnect and session-scoped stat transitions without manual buttons. Separately, close the public docs trim/move/sanitize slice and decide which dashboard pieces remain clean public-core backports versus future private armactl-dashboard baseline. Keep Discord/player enrichment, banlist/moderation, and automatic session scheduling behind the existing truth/recovery gates until that contract is reviewed.
