...
F3-c production acceptance is complete: only the reviewed generated units were explicitly enabled, ingest readiness and ordering were verified, automatic terminal cycles and session transitions were observed, and rollback was retained. Full session detail/API and richer current-session semantics remain separate later work; the first server-rendered detail/search contract is [player-session-detail-search-contract.md](player-session-detail-search-contract.md), with no JSON API in its initial implementation. Exact join time, roles, playtime, K/D, public/player-facing stats, and Discord/public enrichment remain blocked.
...
- slice 5: complete — F3-b synchronous ordered player-session oneshot/timer and F3-c Serhiivka-first, approval-gated Chervonopilya production acceptance; richer session detail/API remains separate later work;
...
- slice 6b: next — query-only detail/search DTO foundation, followed separately by Slice 6c authenticated UI and Slice 6d staged VM smoke;
