...
Full last-seen automation beyond explicit background refresh, the snapshot-only current-roster cache updater, explicit service helper calls, the Phase 4d-a/4d-b stored-log job, and the Phase 4e-a one-shot live scan job, plus current-session semantics, should be added as a later session poller/service slice. That slice should define automatic scheduling, conflict windows, and UI/API truth labels before promoting join time, roles, or combat counters as current-player facts.
