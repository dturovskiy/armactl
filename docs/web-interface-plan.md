...
Community-facing statistics must stay read-only. Public Discord/website-style output can include server status, map/scenario summary, player counts and names, FPS freshness, and mod counts, but must not expose secrets, raw paths, admin actions, or server-control commands. The Discord webhook publisher should create one message and update it on later runs instead of spamming the channel; timestamps should use Discord-native local rendering. Per-user message-language localization is future full-bot scope, not webhook scope.
