...
| --- | --- | --- | --- | --- |
| `Kills` | `player_log_events` stores `kill` and `teamkill` parsed from script `INFO: KILL ENEMY` / `INFO: KILL TK`; optional ServerAdminTools wrapper is only a hint. | Killer is reliable only when the script line includes `instigator_id`/UUID. The authenticated `/players` slice now joins the current roster reliable ID to an open stored `player_sessions` window and counts stored `kill` rows only. Existing `PlayerSummary` counters remain global stored-event counters, not current-session truth. | AI kills have no killer ID and do not count for a player. Suicide is not a kill. Unknown/missing killer is not assignable. Duplicate log collection is mitigated by event keys, and current aggregation counts stored event rows only. Teamkills are intentionally excluded from `Kills` and shown in `TK`. | `implemented for authenticated web-only open-session window` |
| `Deaths` | No separate generic death event exists. Deaths are derived from combat event victim fields on `kill`, `teamkill`, `suicide`, and `other_death`. | Victim is reliable only when `victim_id`/UUID is present. The authenticated `/players` slice uses the same open stored-session window as kills. | AI or environment deaths can still have a player victim and count as death when victim ID is reliable. Unknown victim is not assigned. Suicide counts as a death. Missing or ambiguous event time is not treated as current-session truth. | `implemented for authenticated web-only open-session window` |
| `TK` | `teamkill` event type and `teamkill` boolean are stored from `INFO: KILL TK`; optional ServerAdminTools hint may confirm `friendly: true` but has no reliable IDs by itself. | Count for killer only when `instigator_id` is reliable and inside the open stored-session window. Victim ID is needed for details/audit, but the column counter belongs to the reliable killer. | Do not derive teamkill from matching faction labels alone. Faction/team labels are evidence, not authoritative team truth. Unknown killer, AI, or hint-only wrapper rows cannot increment a player TK counter. | `implemented for authenticated web-only open-session window` |
| `Faction` | `faction_join` stores `faction_resource` and `player_faction`; combat rows store victim/instigator faction labels; `player_sessions` stores optional `faction`/`side` snapshots from evidence. | Current roster rows do not carry faction. The authenticated `/players` slice shows only last stored faction/combat event evidence for the same reliable ID inside the open stored-session window, with last-known evidence wording/tooltip. | Show `—` for unreliable roster rows, missing `players.db`, no open session/evidence, stale/closed-only evidence, or conflicting old evidence. Do not call it current faction unless a future recent reliable source is defined. | `implemented as last-known authenticated web-only evidence` |
...
- Kills count only stored `kill` rows with reliable `instigator_id`; teamkills increment `TK` instead. AI or unknown instigators do not increment a player.
