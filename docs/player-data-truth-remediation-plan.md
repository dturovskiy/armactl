...
F2-b production player-log timer enablement/observation and F3-c supervised player-session production acceptance are complete on Serhiivka and Chervonopilya under the ordered synchronous contract in [player-session-supervised-pipeline-contract.md](player-session-supervised-pipeline-contract.md). Materialized counters, current-roster cache stat persistence, Discord/public combat or faction enrichment, role/loadout display, K/D, ban/kick coupling, public player IDs, and any source that stores or renders raw log lines, raw paths, RCON rows, IPs, or secrets remain blocked. Role remains placeholder-only.
...
The selected F3-b correction is deliberately reuse-based: add one disabled-by-default player-session systemd oneshot/timer that calls a synchronous ordered orchestrator independent of `armactl-web.service`. It must gate on a newly completed fresh ingest generation, then run stored-log sessionization, reliable live-roster scan, and due maintenance under one cross-process lock. Success means actual completion, never enqueue or elapsed time. Existing `web_jobs` remain manual operator workflow metadata. F3-c accepted this on Serhiivka first and then on Chervonopilya after explicit approval, with automatic-cycle observation and rollback proof. See [player-session-supervised-pipeline-contract.md](player-session-supervised-pipeline-contract.md).
