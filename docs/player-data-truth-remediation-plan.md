...
F2-b production player-log timer enablement/observation is complete. F3-b automatic player-session supervision is implemented locally and tested under the ordered synchronous contract in [player-session-supervised-pipeline-contract.md](player-session-supervised-pipeline-contract.md); its generated units remain disabled and uninstalled pending F3-c production enablement, observation, acceptance, and rollback proof. Materialized counters, current-roster cache stat persistence, Discord/public combat or faction enrichment, role/loadout display, K/D, ban/kick coupling, public player IDs, and any source that stores or renders raw log lines, raw paths, RCON rows, IPs, or secrets remain blocked. Role remains placeholder-only.
...
Slice F2-a implements the separate service foundation: generated `armactl-player-log-ingest.service` and `armactl-player-log-ingest.timer` units, explicit install/enable/disable/status commands, a 120-second completion-relative cadence, direct project `.venv` execution, owner/private-permission controls, a 360-second bounded failure guard, restrained CPU/I/O priority, controlled scheduled lock skips, counts-only journald output, and bounded failure/freshness-transition/recovery audit. Installation never enables or starts the timer and preserves an existing timer's enablement state. F2-c adds bounded tail bootstrap and incremental append coverage for an oversized active log without unbounded reads.

F2-b production acceptance is complete. Serhiivka was deployed and observed first; Chervonopilya followed only after approval and used the F2-c bounded catch-up path for its busy active log. Both VMs produced repeated non-overlapping fresh cycles without the manual button, game/web process changes, raw-data journal output, or player-session scheduler enablement. This proves automatic ingest freshness only: current stats still require a proven open play session whose opening is covered by the stored freshness window.

#### Slice F3-a: Supervised Player-Session Pipeline Audit/Design — complete

The existing `armactl players sessions scheduler run --once` is not equivalent to the synchronous F1/F2 ingest runner. It creates or reuses queued `web_jobs` rows and records enqueue as scheduler success, but it does not dispatch a job, start a worker, wait for terminal state, or verify session work. No generic web worker drains those rows; web-process restart does not resume them; and an orphaned queued row can stay active indefinitely because queued rows have no lease or stale-queue recovery. This is a P0 gate before F3 automatic enablement, not an active automatic production incident: no player-session scheduler service/timer is currently installed or enabled.

The selected F3-b correction is deliberately reuse-based: add one disabled-by-default player-session systemd oneshot/timer that calls a synchronous ordered orchestrator independent of `armactl-web.service`. It must gate on a newly completed fresh ingest generation, then run stored-log sessionization, reliable live-roster scan, and due maintenance under one cross-process lock. Success means actual completion, never enqueue or elapsed time. Existing `web_jobs` remain manual operator workflow metadata. F3-c must accept this on Serhiivka first and Chervonopilya only after explicit approval. See [player-session-supervised-pipeline-contract.md](player-session-supervised-pipeline-contract.md).
