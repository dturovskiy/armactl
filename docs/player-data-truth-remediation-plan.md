...
F2-b production player-log timer enablement/observation and F3-c supervised player-session production acceptance are complete on canary server and primary server under the ordered synchronous contract in [player-session-supervised-pipeline-contract.md](player-session-supervised-pipeline-contract.md). Materialized counters, current-roster cache stat persistence, Discord/public combat or faction enrichment, role/loadout display, K/D, ban/kick coupling, public player IDs, and any source that stores or renders raw log lines, raw paths, RCON rows, IPs, or secrets remain blocked. Role remains placeholder-only.
...
F2-b production acceptance is complete. Canary server was deployed and observed first; primary server followed only after approval and used the F2-c bounded catch-up path for its busy active log. Both VMs produced repeated non-overlapping fresh cycles without the manual button, game/web process changes, raw-data journal output, or player-session scheduler enablement. This proves automatic ingest freshness only: current stats still require a proven open play session whose opening is covered by the stored freshness window.

#### Slice F3-a: Supervised Player-Session Pipeline Audit/Design — complete

The legacy enqueue-only scheduler path created or reused queued `web_jobs` rows without proving execution. F3-b closed that P0 architecture gap with the synchronous supervised oneshot/timer; legacy scheduler rows now remain compatibility diagnostics and are not execution truth. F3-c explicitly enabled and accepted the supervised timer on both target VMs.

The selected F3-b correction is deliberately reuse-based: add one disabled-by-default player-session systemd oneshot/timer that calls a synchronous ordered orchestrator independent of `armactl-web.service`. It must gate on a newly completed fresh ingest generation, then run stored-log sessionization, reliable live-roster scan, and due maintenance under one cross-process lock. Success means actual completion, never enqueue or elapsed time. Existing `web_jobs` remain manual operator workflow metadata. F3-c accepted this on canary server first and then on primary server after explicit approval, with automatic-cycle observation and rollback proof. See [player-session-supervised-pipeline-contract.md](player-session-supervised-pipeline-contract.md).
