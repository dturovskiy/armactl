...
> **Примітка:** Номери ТЗ (ТЗ-1, ТЗ-2, …) — це **тематичні розділи специфікації**, а не порядок реалізації. Фактичний порядок робіт зафіксований у секції «Пріоритети розробки» нижче та в окремому файлі `checklist.md`.
>
...
Для Proxmox/multi-VM deployment план web-панелі такий: armactl лишається
всередині кожної game VM поруч із Arma server, а публічний доступ і HTTPS
забезпечує reverse proxy/LXC/VM на окремому рівні.
...
Статус реалізації цього розділу ведеться в `checklist.md` і тут не дублюється.
